...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(' API Route - Filter destinations:', { city, country, type, page, limit })

    // Build the backend URL with filter parameters
    ...
    const backendUrl = `http://localhost:8000/api/v1/destinations/filter?${queryParams.toString()}`
    console.log(' API Route - Backend filter URL:', backendUrl)
    ...
    console.log(' API Route - Backend filter response status:', response.status)
    ...
    if (!response.ok) {
      ...
      console.log(' API Route - Backend filter error response:', errorData)
      
      return NextResponse.json(
        { 
          error: errorData.error || 'Failed to filter destinations',
    ...
    console.log(' API Route - Backend filter success response:', data)
    ...
    if (Array.isArray(data)) {
      ...
      filteredDestinations = data.destinations
    } else if (data.results) {
      // Backend returned with results property
      filteredDestinations = data.results
    }
    
    console.log(' API Route - Filtered destinations count:', filteredDestinations.length)
    console.log(' API Route - Filtered destination types:', filteredDestinations.map((d: any) => d.type))
    
    // Transform backend data to match frontend expectations
    const transformedDestinations = filteredDestinations.map((item: any) => {
      ...
      console.log(` API Route - Filtered destination ${item.name} (${item.city}, ${item.country}): Using static image ${cacheBustedImage}`)
    ...
    console.log(' API Route - Final transformed filtered destinations:', paginatedDestinations.length, 'items')
    console.log(' API Route - Final filtered destination types:', paginatedDestinations.map((d: any) => d.type))
    ...
    console.error(' API Route - Filter destinations route error:', error)
    return NextResponse.json(
      { 
        error: 'Failed to connect to filter service',
